...
import { BookOpen, Download, Calendar, Users, ArrowUpRight, RefreshCw, X } from 'lucide-react';
...
export default function AttendanceRegistryTab() {
  ...
  const handleExportCSV = async () => {
    try {
      const response = await secureFetch('/api/attendance/export');
            ...
            <div className="p-6 overflow-y-auto">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                  ...
                  <span className="text-sm text-slate-500">{detailLoading ? 'Loading...' : `${selectedServiceDetail?.attendance_count || 0} people`}</span>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {(selectedServiceDetail?.attendees || []).map((attendee) => (
                        <tr key={attendee.id}>
                      ...
                      {!detailLoading && (selectedServiceDetail?.attendees || []).length === 0 && (
                        <tr>
